...
func set_shape_precision(precision):
	...
	get_node("container/shape_precision").set_text(str("Shape precision: ", to_percent_string(precision)))

func set_blocks_used(blocks, max_blocks):
	self.blocks = blocks
	self.max_blocks = max_blocks
	get_node("container/blocks_used").set_text(str("Blocks used: ", blocks, "/", max_blocks))

func generate_verdict():
	get_node("verdict").set_text(str('"', precision_verdict(precision), block_verdict(blocks, max_blocks), '"'))
